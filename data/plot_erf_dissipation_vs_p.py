...
for d in dirs:
    ...
    with h5py.File('{:s}/data_top_cz.h5'.format(d), 'r') as f:
        ...
        mean_dissipation = np.mean(enstrophy[good_times])/Re
        mean_z2 = np.mean(z2[good_times])
        mean_vel = np.mean(vel_Ls[good_times])
        mean_fconv = np.mean(fconv[good_times])
        if Re == 6.4e3:
    ...
    data.append((S, P, Re, threeD, mean_L_d01, mean_L_d05, mean_L_d09, mean_f, Ls, Lz, erf, ae, mean_z2, mean_dissipation, mean_vel, mean_fconv))
#    print(d)
...
good = (Re == 4e2)*(S == 1e3)
plt.figure()
#plt.scatter(L_d09[good], vels[good])
#plt.xlabel('L')
#plt.ylabel('u')
plt.scatter(P[good], fconv[good])
plt.ylabel('B')
#plt.scatter(P[good], vels[good]**2)
#plt.scatter(P[good], dissipation[good])

plt.figure()
plt.scatter(P[good], (dissipation/fconv)[good])
plt.ylabel('f')
plt.show()
