...
\editone{
    ...
    The remaining three parameters are $\mS$, $\mP$, and an additional parameter $\eta$, which we will hold constant and which sets the ratio between $\gradrad$ and $\gradad$ in the convection zone (see discussion following Eqn.~\ref{eqn:sim_Q}).
}

We nondimensionalize Eqns.~\ref{eqn:incompressible}-\ref{eqn:boussinesq} on the length scale of the Schwarzschild-unstable convection zone $L_s$, the timescale of freefall across that convection zone $\tau_{\rm{ff}}$, and the temperature scale of the internal heating over that freefall time $\Delta T$; \editone{mass is nondimensionalized so that the freefall ram pressure $\rho_0(L_s/\tau_{ff})^2$ is one,}
\begin{equation}
\begin{split}
...
Aside from $\mS$, $\mP$, and $\eta$, the two remaining control parameters $\mR$ and $\Pran$ determine the properties of the turbulence.
