...
Here we examine how convective motions efficiently mix entropy in the radiative zone.
We present 3D numerical simulations which exhibit penetration zones: regions where the entire luminosity \emph{could} be carried by radiation, but where the temperature gradient is approximately adiabatic and convection is present.
...
We conduct 3D simulations with Dedalus using a simplified Boussinesq model of stellar convection including a radiative flux, and we find good agreement between simulations and the derived theory.
...
We present a MESA solar model which employs our parameterization of convective penetration, and find that the base of the solar convection zone extends by $\sim$35\% of a pressure scale height.
...
Many studies have exhibited hints of penetrative convection -- that is, the mixing of the entropy towards an adiabatic profile beyond the convective boundary.
Some of these studies exhibit dynamical or flux-based hints of penetration (the convective flux is negative and the radiative flux carries more than the total flux going through the simulation), but do not clearly report the value of the entropy gradient \citep{hurlburt_etal_1986, singh_etal_1995, browning_etal_2004, brun_etal_2017, pratt_etal_2017}
Others report a change of the entropy gradient towards the adiabatic beyond the convecting region \citep{hurlburt_etal_1994, saikia_etal_2000, rogers_etal_2006, kitiashvili_etal_2016}
Some simulations show little evidence of true penetration \citep[see e.g., the superadiabaticity profiles of][]{higl_etal_2021}.
...
\section{Theory}
\label{sec:theory}
In this section we derive a theoretical model of convective penetration by examining the energetics and energy fluxes in the convection and penetration zones.
...
By balancing the excess KE in the convection zone with a buoyancy-braking work term in the PZ, we are able to derive the size of the PZ.
...
We see that the addition of a PZ increases the sound speed by $\mathcal{O}$(1-2\%) below the convection zone.
...
We see that the inclusion of a PZ leads to an $\mathcal{O}$(1-2\%) increase in the sound speed profile near the base of the solar convection zone, similar to what is observed between helioseismic observations and standard models \citep{bergemann_serenelli_2014}.
