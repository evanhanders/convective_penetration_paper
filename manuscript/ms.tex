...
\subsection{Kinetic energy \& the vorticity-flux link}
...
Assuming a statistically stationary state, and assuming that the kinetic energy fluxes vanishes at the upper and lower boundaries of the convection zone, this equation permits one solution,
\begin{equation}
...
It is counterintuitive to find that a viscous term is on the same order of magnitude as the convective flux.
However, this term dimensionally behaves like $\nu u^2 / \ell^2$, where $\ell$ is the length scale of velocity gradients; if the average vorticity is dominated by gradients at the dissipation scale, then it makes sense that this term can persist even at small values of $\nu$.
Furthermore, \citet{currie_browning_2017} demonstrated that even in appreciably density-stratified atmospheres, the magnitude of the viscous heating term remains large for turbulent flows.
Since $\nu |\vec{\omega}^2|$ is the Boussinesq analog to the viscous heating, the importance of this term is perhaps not too surprising.
...
Following Eqn.~\ref{eqn:ke_averages}, we can write
\begin{equation}
\int_{\rm{CZ}} \bar{\Fconv} dz + \int_{\rm{PZ}} \bar{\Fconv} dz = \frac{\nu}{\alpha g}(L_s + \delp) \angles{|\vec{\omega}|^2}.
...
\qquad\rm{with}\qquad
f \equiv \frac{\nu}{\alpha g}(L_s + \delp) \frac{\angles{|\vec{\omega}|^2}}{\int_{\rm{CZ}} \bar{\Fconv} dz}
...
The upshot of this equation is that the ratio of the flux integrals in the PZ and CZ relates to some characteristic properties of the convective dynamics.
In order to say more, we must turn to a more in-depth examination of the fluxes in the PZ.
...
With the knowledge that $\Fconv$ is negative in the PZ, we rewrite Eqn.~\ref{eqn:preliminary_flux_ratio}
\begin{equation}
\frac{|\int_{\rm{PZ}}\bar{\Fconv}\,dz|}{\int_{\rm{CZ}}\bar{\Fconv}\,dz} = (1 - f),
\label{eqn:theory_fraction}
\end{equation}
and note that $f < 1$ by its flux-based definition.
While we have arrived here in a very different manner, this equation represents the central result of \citet{zahn1991}'s theory: the ratio of the vertically integrated convective fluxes in the PZ and CZ is directly linked to some constant which relates to the convective dynamics.
...
Furthermore, if $\mP_D$ doubles, the PZ magnitude of $\Fconv$ halves, and if $f$ does not change then the PZ must also double in size according to Eqn.~\ref{eqn:ke_averages}.
...
We measure the time-evolution of the convective front $\frac{d \delp}{dt}$ by averaging the slope of the linear fits for $\delta_{0.1}$ and $\delta{0.2}$.
...
\item  If $\Delta \delta_p$ is large, we adjust the top of the PZ by setting $\delta_{0.5,\rm{new}} = \angles{\delta_{0.5}}_t + \Delta \delta_p$.
If $\Delta \delta_p > 0.05$, we limit its value to 0.05.
We calculate the width of the thermal adjustment layer $\delta_w$ as the minimum of $\angles{\delta_{0.9} - \delta_{0.5}}_t$ and $\angles{\delta_{0.5} - \delta_{0.1}}_t$.
We adjust the $m = 0$ temperature gradient to
\begin{equation}
\partial_z \overline{T} = -\gradad - H(z; \delta_{\rm{0.5,\rm{new}}}, \delta_w) \Delta\grad,
