...
\editone{
    At the top of the PZ, between $1.3 \lesssim z \lesssim 1.4$, there is a ``thermal adjustment layer'' where convective motions give way to conductive transport and $\justgrad$ adjusts from $\gradad$ to $\gradrad$.
...
We plot $\justgrad$ (green) compared to $\gradad$ (purple, a constant) and $\gradrad$ (orange); note the extended penetration zone (PZ) where $\justgrad \approx \gradad > \gradrad$.
    \editone{
        The dashed vertical line denotes the Schwarzschild boundary of the convection zone (CZ), the greyed region denotes the thermal adjustment layer, and the solid vertical line denotes the bottom of the radiative zone (RZ).
    }
...
\editone{
    In Eqns.~\ref{eqn:incompressible}-\ref{eqn:boussinesq}, there are nine fundamental input parameters: $\rho_0$, $\alpha$, $g$, $\nu$, $\chi$, $Q$, $\gradad$, $k_{\rm{CZ}}$, and $k_{\rm{RZ}}$.
    ...
    The remaining three parameters are $\mS$, $\mP$, and an additional parameter ``$\eta$,'' which we will hold constant and which sets the value of the flux entering the bottom boundary.
}
...
\editone{
    ...
    \begin{equation}
        \mS_{\rm{alt}} = \frac{|\gradrad - \gradad|_{\rm{RZ}}}{|\gradrad - \gradad|_{\rm{CZ}}} = \frac{N^2}{|\gradrad - \gradad|_{\rm{CZ}}}.
    \end{equation}
    In those studies, $\mS_{\rm{alt}}$ primarily describes the stratification of the initial state, but it also describes the stratification near superadiabatic boundary layers which drive convection.
    ...
    \begin{equation}
        ...
        %= \frac{k_{\rm{CZ}}}{k_{\rm{RZ}} \mS_{\rm{alt}}} = \frac{k_{\rm{CZ}}(\gradrad - \gradad)_{\rm{CZ}}}{k_{\rm{RZ}} \mS}.
    \end{equation}
    We note that $\mP$ can be related to $\mS$ and $\mS_{\rm{alt}}$, $\mP = (k_{\rm{CZ}}/k_{\rm{RZ}}) \mS_{\rm{alt}}^{-1} = (k_{\rm{CZ}}/k_{\rm{RZ}}) (\gradrad - \gradad)_{\rm{CZ}} \mS^{-1}$.
...
\editone{
    We note briefly that the integral formulation of the equations in e.g., Eqn.~\ref{eqn:mesa_eqn} is not aware of the topology of convective perturbations, which depends upon the density stratification across the convective domain.
    We suspect that relaxing the Boussinesq approximation and accounting for stratification changes the theory by adding explicit stratification terms into the integral constraint, but this derivation is beyond the scope of this work. 
}
